...
def default_plots_grid(logger, output_dir=None, conf = None, to_grid_parameters={}, fitness_domain=None):
    ...
    plot_iterations(logger, os.path.join(output_dir, "./p_valid.pdf"), "p_valid", ylabel="Number of valid solutions")
    plot_iterations(logger, os.path.join(output_dir, "./max_ss.pdf"), "max_ss", ylabel="Best safety score")
    plot_iterations(logger, os.path.join(output_dir, "./mean_ss.pdf"), "mean_ss", ylabel="Mean safety score")

    if conf['action_features']:
        return
    ...
    #print(grid.safety_array)
    plot_path = os.path.join(output_dir, "Safety_Grid.pdf")
    plotGridSubplots(grid.safety_array[... ,0], plot_path, plt.get_cmap(cmap_perf), grid.features_domain,[0,12,], nbTicks=None)
