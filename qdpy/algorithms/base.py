...
class QDAlgorithm(abc.ABC, QDAlgorithmLike, Summarisable, Saveable, Copyable, CreatableFromConfig):
    ...
    def tell(self, individual: IndividualLike, fitness: Optional[Any] = None,
             features: Optional[FeaturesLike] = None, valid: Optional[Any] = None, ss = None,  elapsed: Optional[float] = None,
    ...
    def optimise(self, evaluate: Callable, budget: Optional[int] = None, 
            batch_mode: bool = True, executor: Optional[ExecutorLike] = None,
                 send_several_suggestions_to_fn: bool = False, config = None) -> IndividualLike:
        """TODO"""
            ...
            while remaining_evals > 0 or len(futures) > 0:
                ...
                if send_several_suggestions_to_fn:
                    ...
                    for _ in range(nb_suggestions):
                        ...
                        futures[eval_id] = _executor.submit(_evalWrapper, eval_id, evaluate, ind, config)
                        remaining_evals -= 1
                    ...
                    if ind_exc is None:
                        if isinstance(ind_res, IndividualLike):
                            ...
                            ind_fitness, ind_features, ind_valid, ind_ss = ind_res
                            ind = individuals[ind_id] # type: ignore
                            self.tell(ind, fitness=ind_fitness, features=ind_features, valid = ind_valid, ss = ind_ss, elapsed=ind_elapsed)
                    else:
                        warnings.warn(f"Individual evaluation raised the following exception: {ind_exc} !")
                        self._nb_evaluations += 1
    ...
    def add_callback(self, event: str, fn: Callable) -> None:
        ...
        #self._callbacks.setdefault(event, []).append(fn)Characterizations
...
#        return ind
    ...
    def tell(self, individual: IndividualLike, fitness: Optional[Any] = None,
             features: Optional[FeaturesLike] = None,valid = None,  elapsed: Optional[float] = None,
            tell_container = False) -> bool:
        added: bool = self.current.tell(individual, fitness, features,valid, elapsed)
