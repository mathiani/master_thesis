...
class AlgorithmLogger(Saveable):
    ...
    _cols_names: Sequence[str] = ["alg_name","iteration", "cont_size", "evals", "nb_updated", "avg", "std", "min", "max", "ft_min", "ft_max","p_valid", "max_ss","mean_ss", "qd_score","qd_score_ss", "elapsed"]
    ...
    def _stats(self, algo: QDAlgorithmLike, batch_elapsed: float) -> Sequence[Any]:
        ...
        for ind in algo.container:
            if ind.valid:
                nr_valid += 1
            if ind.ss > max_ss:
                max_ss = ind.ss
            mean_ss += ind.ss
        mean_ss = mean_ss/len(algo.container)
        p_valid = nr_valid/len(algo.container)
        mean_ss = np.round(mean_ss, 2)
        max_ss = np.round(max_ss, 2)
        p_valid = np.round(p_valid, 2)
        ...
        qd_score, qd_score_ss = algo.container.qd_score(normalized=False)
        qd_score = int(qd_score)
        qd_score_ss = int(qd_score_ss)
        batch_elapsed_str = f"{batch_elapsed:.2f}"
        with np.printoptions(precision=2, suppress=True):
            res = list(map(str, [alg_name, iteration, cont_size, evals, nb_updated, avg_values, std_values, min_values, max_values, ft_min_values, ft_max_values, p_valid,max_ss,mean_ss , qd_score, qd_score_ss, batch_elapsed_str]))
        return res
    ...
    class TQDMAlgorithmLogger(AlgorithmLogger):
        ...
        def _vals_to_cols_title(self, content: Sequence[Any]) -> str:
            if _IMPORTED_COLORAMA:
                assert len(content) == len(self._cols_size)
                prefix = "" + colorama.Style.BRIGHT + colorama.Back.GREEN
