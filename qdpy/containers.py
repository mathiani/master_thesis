...
from qdpy.metrics import novelty_local_competition, novelty_nn
...
class Container(Sequence, Summarisable, Copyable, CreatableFromConfig):
    ...
    def best_fitness(self) -> Optional[FitnessLike]:
        """Return the fitness values of the individual with the best qualitty, or None. """
...
#        return reversed(self.items)
    ...
    def _discard_by_index(self, individual: IndividualLike, idx: Optional[int] = None, idx_depot: Optional[int] = None, also_from_depot: bool = True) -> None:
    ...
    def qd_score(self, normalized: bool = False) -> float:
...
class Grid(Container):
    ...
    def _init_grid(self) -> None:
        ...
        self._ss = {x: [] for x in self._index_grid_iterator()}
        ...
        self._safety_array =  np.full(self._shape + (len(self.fitness_domain),), np.nan)
    ...
    @property
    def safety_array(self):
        return self._safety_array
    ...
    def _update_quality(self, ig: GridIndexLike) -> None:
        ...
        if self._nb_items_per_bin[ig] == 0:
            ...
            val2: Optional[FitnessLike] = None
        ...
        if val is None:
            ...
            self.safety_array[ig] = math.nan
        else:
            self.quality_array[ig] = val.values
            self.safety_array[ig] = val2
    ...
    def add(self, individual: IndividualLike, raise_if_not_added_to_depot: bool = False) -> Optional[int]:
        ...
        if can_be_added:
            ...
            if index == old_len: # Individual was not already present in container
                ...
                self._ss[ig].append(individual.ss)
...
class NoveltyArchive(Container):
    ...
    def __init__(self, iterable: Optional[Iterable] = None,
                 k: int = 15, threshold_novelty: float = 0.01, NS = False , novelty_distance: Union[str, Callable] = "euclidean",
    ...
    def add(self, individual: IndividualLike, raise_if_not_added_to_depot: bool = True) -> Optional[int]:
        ...
        if novelty > self.threshold_novelty:
            ...
            out = self._add_internal(individual, raise_if_not_added_to_depot, False)
            #print("ADDED")
            #print(out)
            if out != -10:
                return out
        ...
        if self.NS:
            ...
            nn_novelty, _ = novelty_nn(ind_nn, self.depot, k=self.k, nn_size=1, dist = self.novelty_distance, ignore_first=False)

            #print(individual.features.values, ind_nn.features.values)

            #print(novelty, nn_novelty)
            
            if novelty > nn_novelty:
                #print("yes")
                self._discard_by_index(ind_nn, idx_depot=nn[0], also_from_depot = True)
                return self._add_internal(individual, raise_if_not_added_to_depot, False)
